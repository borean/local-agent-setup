---
name: power-analysis
description: G*Power-equivalent in R (`pwr`, `WebPower`, `simr` for mixed models). Computes n given effect+alpha+power OR detectable effect given n.
domain: research
pillar: statistics
user-invocable: true
target_models:
...
G*Power-equivalent in R (`pwr`, `WebPower`, `simr` for mixed models). Computes n given effect+alpha+power OR detectable effect given n.
...
Wraps `pwr` and `WebPower` R packages.
