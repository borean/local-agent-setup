...
description: Inspect CSV/XLSX. Propose variable types. Flag suspicious values. Return clean data dictionary YAML. Top-20 rows max — never reads the full dataset.
domain: research
pillar: statistics
user-invocable: true
target_models:
...
Inspect CSV/XLSX. Propose variable types. Flag suspicious values. Return clean data dictionary YAML. Top-20 rows max — never reads the full dataset.
...
Standard data-prep pattern. The 'top-20 rows only' rule is a PHI-safety guarantee.
