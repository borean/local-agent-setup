---
name: analysis-run
description: Emit R/Python code for a chosen analysis from the plan, execute it via Quarto/Jupyter, capture outputs, return narrative.
domain: research
pillar: statistics
user-invocable: true
target_models:
...
Emit R/Python code for a chosen analysis from the plan, execute it via Quarto/Jupyter, capture outputs, return narrative.
...
R via `mall` package (mlverse). Python via Jupyter+ollama client. Local Qwen does code emission + interpretation.
