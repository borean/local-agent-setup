---
name: leann-search
description: Semantic search over local Zotero corpus via LEANN. Returns top-K chunks with file path + page + score.
domain: research
pillar: literature
user-invocable: true
target_models:
...
Semantic search over local Zotero corpus via LEANN. Returns top-K chunks with file path + page + score.
...
LEANN by @LiorOnAI (surfaced in WhatsApp data); 60M chunks in 6GB by storing graph not vectors.
